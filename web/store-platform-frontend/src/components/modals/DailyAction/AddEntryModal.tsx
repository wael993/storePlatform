...
const AddEntryModal = ({ isOpen, onClose }: AddEntryModalProps) => {
	...
	const entryTypeOptions: DropdownOption[] = useMemo(
		() => [
			{ value: DailyActionType.RECEIPT_ENTRY, label: t('common.receiptEntry') },
			{ value: DailyActionType.PAYMENT_ENTRY, label: t('common.paymentEntry') },
			{ value: DailyActionType.EXPENSE_ENTRY, label: t('common.expenseEntry') },
		],
		[t],
	)
	...
	const isSubmitButtonDisabled = isSavingDailyAction || !formData?.invoiceDate
				...
				<ModalContent>
					<ModalHeader sx={styles.header}>
						<VStack sx={styles.headerTitleStepperContainer}>
							<Text sx={styles.headerText}>
								{t('components.sellingInvoices.entries.title')}
							</Text>
					...
					<ModalBody sx={styles.body}>
						...
						{step === StepKeys.ACTION_TYPE && (
							<FirstStep
								actionEntryTypesOptions={entryTypeOptions}
								entryType={entryType}
