...
export const config = {
	...
	actionsEnabled: splitByComma(
		process.env.VITE_ACTIONS_ENABLED ??
			'PRODUCTS,ADD_NEW_TENANT,TENANTS_LIST,DAILY,CUSTOMERS,CATEGORIES,SUPPLIERS,PARTNERS,SETTINGS,USERS,BARCODE,SELLING_INVOICES', //,,ORDERS,INVOICE,USERS,CHANGE_PASSWORD,SETTINGS', //'$VITE_ACTIONS_ENABLED',
	),
	tenantActions: splitByComma(
		process.env.VITE_TENANT_ACTIONS_ENABLED ??
			'BARCODE,PRODUCTS,ORDERS,INVOICE,USERS,ADD_NEW_TENANT,TENANTS_LIST,CHANGE_PASSWORD,DAILY,CUSTOMERS,CATEGORIES,SUPPLIERS,PARTNERS,SETTINGS,CATEGORIES,SELLING_INVOICES',
	),
}
